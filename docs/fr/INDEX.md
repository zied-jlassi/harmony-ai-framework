# Documentation Harmony Framework

> **🌐 Langue :** [English](../INDEX.md) · Français
...
- [Hooks](hooks.md) - **Référentiel des hooks** : les 11 hooks, contrat stdin/exit 2, visibilité (`systemMessage`), défaut vs optionnel
- [Configuration](configuration.md) - **Référentiel de configuration** : toutes les variables `HARMONY_*` et les fichiers de config
...
- [How It Works](how-it-works.md) - Fonctionnement interne (routing intelligent, **Observable by design**, JIT context)
...
- [Security Guards](security-guards.md) - Couche de protection active par défaut : **rules-enforcer** (commandes destructrices, injection shell), **supply-chain**, **anti-injection LLM** — avec **preuve visible** à chaque déclenchement (`systemMessage`)
...
| **Context Preloader** | Chargement securise avec state machine (anti-boucle infinie) + router RouteLLM **config-driven** (priorité Claude Code natif > clé API > pattern ; résumé `📥 Context` visible) |
