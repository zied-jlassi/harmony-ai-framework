...
<critical>You MUST have already loaded and processed: {project-root}/.harmony/workflows/4-implementation/sprint-status/workflow.yaml</critical>
...
Run `/harmony:workflows:sprint-planning` to generate it, then rerun sprint-status.</output>
...
<step n="2" goal="Read and parse sprint-status.yaml">
  ...
  - Stories in review but no reviewer assigned context → suggest `/harmony:workflows:code-review`
  - Stories in in-progress with no ready-for-dev items behind them → keep focus on the active story
  - All epics backlog/contexted but no stories drafted → prompt to run `/harmony:workflows:create-story`
</step>
...
**Next Recommendation:** /harmony:workflows:{{next_workflow_id}} ({{next_story_id}})
...
Choice:</ask>
  ...
  <check if="choice == 1">
    <output>Run `/harmony:workflows:{{next_workflow_id}}`.
