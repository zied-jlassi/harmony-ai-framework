...
> **🌐 Langue :** [English](../../architecture/context-loading-system.md) · Français
...
## Exemple concret: Gaming Mobile Auth
...
## Référence : Context Flags (preloader)

Le preloader (`lib/context-preloader.sh`) détecte des **context flags** à partir de la requête, puis charge le knowledge JIT correspondant et déclenche éventuellement un agent de validation parallèle. Mapping faisant foi : `_mock_classification()` (mots-clés) et `_get_knowledge_for_flag()` (knowledge).

| Flag | Mots-clés déclencheurs | Knowledge chargé (JIT) | Agent déclenché |
|------|------------------------|------------------------|-----------------|
...
> Note : ces flags du **preloader** sont distincts des flags de **routage Guardian** définis dans `config/routing-rules.yaml` (`has_ui`, `has_db_schema`, `security_critical`, `has_minors`, `has_media`, `has_social`, `legal_compliance`…), utilisés par l'aria-detector pour le routage d'agents.

### Détection de profils (depuis les fichiers projet)

En complément des flags, le preloader détecte le tech stack via `_detect_profiles_flat()` :

| Fichier projet | Profil détecté |
|----------------|----------------|
