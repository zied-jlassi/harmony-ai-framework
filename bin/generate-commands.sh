...
if [[ -f "$COMMANDS_DIR/agents.yaml" ]]; then
    ...
    generate_agent_command "ai" "agents/specialists/ai-architect.md" "AI/LLM architect - RAG, multi-agent, memory systems" "[ai-topic]"
    generate_agent_command "qa" "agents/specialists/exploratory-qa.md" "Exploratory QA - manual testing, UX validation" "[module-or-feature]"
    generate_agent_command "ucv-writer" "agents/specialists/ucv-writer.md" "UCV Writer - creates Use Case Verifiables (HQVF)" "[story-id]"
    generate_agent_command "ucv-validator" "agents/specialists/ucv-validator.md" "UCV Validator - validates 100% UCV coverage (HQVF)" "[story-id]"
